...
## Docker + K8s 分离部署（小型练习推荐 👍）

GZCTF、数据库在一个 Docker 实例中，并使用远程 k8s 作为题目容器平台。这样的部署方式可以在可接受的成本下最大化地利用 k8s 的安全性和部署优势，部署过程更加亲民。**采用 k3s 作为 k8s 发行版，可以实现低难度部署，对于一般 CTF 练习和校赛来说是一个不错的选择**。[查看细节](/zh/deployment/docker-k8s)

...
此处配置加密密钥，用于加密数据库中练习的私钥信息，可为任意长度的任意字符串。

### ContainerProvider

此处配置容器后端，为练习动态创建容器的必填项。

- **Type:** 容器后端类型，可选 `Docker` 或 `Kubernetes`。
- **PublicEntry:** 容器后端的公网地址，用于生成练习的访问地址，展示给参赛队伍。
...
此处配置 Docker Registry 的用户名和密码，用于练习动态创建容器时拉取镜像的验证，可选项。

...
本文档不涉及如何配置 HTTPS，关于 nginx、traefik 等软件的配置使用方法请自行查询，请注意 GZCTF 的 websocket 功能可能也需要进行独立配置，如遇到练习通知无法推送等问题请先排查反向代理是否配置正确。
...
1. 为了后续的配置，请准备好如下的初始化参数：
   ...
   - `XOR_KEY`: 用于加密练习私钥的随机字符串，需要配置在 `appsettings.json` 中。
...
也可以参考和借鉴 [W4terCTF 2023](https://github.com/W4terDr0p/W4terCTF-2023) 的公开题目仓库，此内的题目均在题目正常工作的情况下尽量保证 Docker Image 的体积足够小，以便于在练习中快速部署。
